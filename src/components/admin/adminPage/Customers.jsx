...
function Customers() {
  ...
  console.log("🚀 ~ Customers ~ employeesData:", employeesData)
  const taskShow = employeesData?.find((e) => e.task_show)
  console.log("🚀 ~ Customers ~ taskShow:", taskShow)
              ...
              {employeesData.map(c => (
                <tr key={c.id} className="border-b hover:bg-gray-50 text-sm">
                  ...
                  <td className="px-4 py-3">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${c.status === 'Active' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-500'
                      }`}>
                      {c.password}
                    </span>
                  </td>
